<nav class="row main-nav expanded collapse">
    <div class="large-6 medium-12 columns">
        <div class="row site-nav">
            <div class="medium-6 small-10 columns">logo</div>
            
            <div class="small-2 columns show-for-small-only" id="site-menu">menu</div>
            <div class="medium-3 columns hide-for-small-only site-menu">who's my mp</div>
            <div class="medium-3 columns hide-for-small-only site-menu">compare parties</div>
                
        </div>
    </div>
    <div class="large-6 columns medium-12 hide-for-small-only">
        <div class="row party-nav">
            <div class="medium-3 columns party-box party"></div>
            <div class="medium-3 columns party-box party"></div>
            <div class="medium-3 columns party-box party"></div>
            <div class="medium-3 columns party-box party"></div>
        </div>
    </div>
</nav>